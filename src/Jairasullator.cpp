 ...
 public:
    ...
    Jairasullator() : ChipModule<GeneralInstrumentAy_3_8910>(2.5) {
        ...
        for (unsigned oscillator = 0; oscillator < GeneralInstrumentAy_3_8910::OSC_COUNT; oscillator++) {
            ...
            configSwitch(PARAM_TONE  + oscillator,        0, 1, 1, name + " Tone",     {"Off", "On"});
            configSwitch(PARAM_NOISE + oscillator,        0, 1, 0, name + " Noise",    {"Off", "On"});
            configSwitch(PARAM_ENVELOPE_ON + oscillator,  0, 1, 0, name + " Envelope", {"Off", "On"});
        }
        ...
        configButton(PARAM_ENVELOPE_MODE, "Envelope Mode");
    }
