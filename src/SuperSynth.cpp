...
struct SuperSynth : Module {
 private:
    /// the RAM for the S-DSP chip (64KB = 16-bit address space)
    uint8_t ram[SonyS_DSP::Processor::SIZE_OF_RAM];
    /// the Sony S-DSP sound chip emulator
    SonyS_DSP::Processor apu{ram};
 ...
 public:
    ...
    enum ParamIds {
        ENUMS(PARAM_FREQ,          SonyS_DSP::Processor::VOICE_COUNT),
        ENUMS(PARAM_PM_ENABLE,     SonyS_DSP::Processor::VOICE_COUNT),
        ENUMS(PARAM_NOISE_ENABLE,  SonyS_DSP::Processor::VOICE_COUNT),
        PARAM_NOISE_FREQ,
        ENUMS(PARAM_VOLUME_L,      SonyS_DSP::Processor::VOICE_COUNT),
        ENUMS(PARAM_VOLUME_R,      SonyS_DSP::Processor::VOICE_COUNT),
        ENUMS(PARAM_ATTACK,        SonyS_DSP::Processor::VOICE_COUNT),
        ENUMS(PARAM_DECAY,         SonyS_DSP::Processor::VOICE_COUNT),
        ENUMS(PARAM_SUSTAIN_LEVEL, SonyS_DSP::Processor::VOICE_COUNT),
        ENUMS(PARAM_SUSTAIN_RATE,  SonyS_DSP::Processor::VOICE_COUNT),
        ENUMS(PARAM_ECHO_ENABLE,   SonyS_DSP::Processor::VOICE_COUNT),
        PARAM_ECHO_DELAY,
        PARAM_ECHO_FEEDBACK,
        ENUMS(PARAM_VOLUME_ECHO, 2),
        ENUMS(PARAM_VOLUME_MAIN, 2),
        ENUMS(PARAM_FIR_COEFFICIENT, SonyS_DSP::Processor::FIR_COEFFICIENT_COUNT),
        NUM_PARAMS
    };

    /// the indexes of input ports on the module
    enum InputIds {
        ENUMS(INPUT_VOCT,          SonyS_DSP::Processor::VOICE_COUNT),
        ENUMS(INPUT_FM,            SonyS_DSP::Processor::VOICE_COUNT),
        ENUMS(INPUT_PM_ENABLE,     SonyS_DSP::Processor::VOICE_COUNT),
        ENUMS(INPUT_NOISE_ENABLE,  SonyS_DSP::Processor::VOICE_COUNT),
        INPUT_NOISE_FM,
        ENUMS(INPUT_GATE,          SonyS_DSP::Processor::VOICE_COUNT),
        ENUMS(INPUT_VOLUME_L,      SonyS_DSP::Processor::VOICE_COUNT),
        ENUMS(INPUT_VOLUME_R,      SonyS_DSP::Processor::VOICE_COUNT),
        ENUMS(INPUT_ATTACK,        SonyS_DSP::Processor::VOICE_COUNT),
        ENUMS(INPUT_DECAY,         SonyS_DSP::Processor::VOICE_COUNT),
        ENUMS(INPUT_SUSTAIN_LEVEL, SonyS_DSP::Processor::VOICE_COUNT),
        ENUMS(INPUT_SUSTAIN_RATE,  SonyS_DSP::Processor::VOICE_COUNT),
        ENUMS(INPUT_ECHO_ENABLE,   SonyS_DSP::Processor::VOICE_COUNT),
        INPUT_ECHO_DELAY,
        INPUT_ECHO_FEEDBACK,
        ENUMS(INPUT_VOLUME_ECHO, 2),
        ENUMS(INPUT_VOLUME_MAIN, 2),
        ENUMS(INPUT_FIR_COEFFICIENT, SonyS_DSP::Processor::FIR_COEFFICIENT_COUNT),
        NUM_INPUTS
    };
    ...
    SuperSynth() {
        ...
        for (unsigned osc = 0; osc < SonyS_DSP::Processor::VOICE_COUNT; osc++) {
 ...
 protected:
    ...
    inline void setupSourceDirectory() {
        ...
        apu.write(SonyS_DSP::Processor::ECHO_BUFFER_START_OFFSET, 128);
        ...
        apu.write(SonyS_DSP::Processor::OFFSET_SOURCE_DIRECTORY, ECHO_LENGTH / 0x100);

        for (unsigned voice = 0; voice < SonyS_DSP::Processor::VOICE_COUNT; voice++) {
            ...
            // $x4   |                 Source Number                 |
            //       +-----+-----+-----+-----+-----+-----+-----+-----+
            apu.write(mask | SonyS_DSP::Processor::SOURCE_NUMBER, 0);
        }
    }
    ...
    inline void process(const ProcessArgs &args) final {
        ...
        auto dir = reinterpret_cast<SonyS_DSP::SourceDirectoryEntry*>(&ram[0x7800]);
        ...
        auto block = reinterpret_cast<SonyS_DSP::BitRateReductionBlock*>(&ram[0x7804]);
        ...
        apu.write(SonyS_DSP::Processor::FLAGS, noise);
        // -------------------------------------------------------------------
        // MARK: Gate input
        // -------------------------------------------------------------------
        ...
        if (key_on) {  // a key-on event occurred from the gate input
            ...
            apu.write(SonyS_DSP::Processor::KEY_OFF, 0);
            // write the key-on value to the register
            apu.write(SonyS_DSP::Processor::KEY_ON, key_on);
        }
        if (key_off)  // a key-off event occurred from the gate input
            apu.write(SonyS_DSP::Processor::KEY_OFF, key_off);
        // -------------------------------------------------------------------
        // MARK: Echo Parameters
        // -------------------------------------------------------------------
        apu.write(SonyS_DSP::Processor::ECHO_FEEDBACK, params[PARAM_ECHO_FEEDBACK].getValue());
        apu.write(SonyS_DSP::Processor::ECHO_DELAY, params[PARAM_ECHO_DELAY].getValue());
        // echo enable
        uint8_t echo_enable = 0;
        for (unsigned voice = 0; voice < SonyS_DSP::Processor::VOICE_COUNT; voice++)
            echo_enable |= static_cast<uint8_t>(params[PARAM_ECHO_ENABLE + voice].getValue()) << voice;
        apu.write(SonyS_DSP::Processor::ECHO_ENABLE, echo_enable);
        // -------------------------------------------------------------------
        // MARK: Noise Enable
        // -------------------------------------------------------------------
        ...
        apu.write(SonyS_DSP::Processor::NOISE_ENABLE, noise_enable);
        // -------------------------------------------------------------------
        // MARK: Pitch Modulation
        // -------------------------------------------------------------------
        uint8_t pitch_modulation = 0;
        // start from 1 because there is no pitch modulation for the first channel
        for (unsigned voice = 1; voice < SonyS_DSP::Processor::VOICE_COUNT; voice++)
            pitch_modulation |= static_cast<uint8_t>(params[PARAM_PM_ENABLE + voice].getValue()) << voice;
        apu.write(SonyS_DSP::Processor::PITCH_MODULATION, pitch_modulation);
        // -------------------------------------------------------------------
        // MARK: Main Volume & Echo Volume
        // -------------------------------------------------------------------
        apu.write(SonyS_DSP::Processor::MAIN_VOLUME_LEFT,  params[PARAM_VOLUME_MAIN + 0].getValue());
        apu.write(SonyS_DSP::Processor::MAIN_VOLUME_RIGHT, params[PARAM_VOLUME_MAIN + 1].getValue());
        apu.write(SonyS_DSP::Processor::ECHO_VOLUME_LEFT,  params[PARAM_VOLUME_ECHO + 0].getValue());
        apu.write(SonyS_DSP::Processor::ECHO_VOLUME_RIGHT, params[PARAM_VOLUME_ECHO + 1].getValue());
        // -------------------------------------------------------------------
        ...
        for (unsigned voice = 0; voice < SonyS_DSP::Processor::VOICE_COUNT; voice++) {
            ...
            apu.write(mask | SonyS_DSP::Processor::PITCH_LOW,  0xff &  pitch16bit     );
            apu.write(mask | SonyS_DSP::Processor::PITCH_HIGH, 0xff & (pitch16bit >> 8));
            // ---------------------------------------------------------------
            ...
            //
            // apu.write(mask | SonyS_DSP::Processor::GAIN, 64);
            // ---------------------------------------------------------------
            // MARK: ADSR
            // ---------------------------------------------------------------
            ...
            apu.write(mask | SonyS_DSP::Processor::ADSR_1, adsr1);
            ...
            apu.write(mask | SonyS_DSP::Processor::ADSR_2, adsr2);
            // ---------------------------------------------------------------
            // MARK: ADSR Output
            // ---------------------------------------------------------------
            ...
            // apu.read(mask | SonyS_DSP::Processor::ENVELOPE_OUT, 0);
            // ---------------------------------------------------------------
            ...
            //
            // 8-bit signed value
            // apu.read(mask | SonyS_DSP::Processor::WAVEFORM_OUT, 0);
            // ---------------------------------------------------------------
            // MARK: Amplifier Volume
            // ---------------------------------------------------------------
            apu.write(mask | SonyS_DSP::Processor::VOLUME_LEFT,  params[PARAM_VOLUME_L + voice].getValue());
            apu.write(mask | SonyS_DSP::Processor::VOLUME_RIGHT, params[PARAM_VOLUME_R + voice].getValue());
        }
        // -------------------------------------------------------------------
        ...
        for (unsigned coeff = 0; coeff < SonyS_DSP::Processor::FIR_COEFFICIENT_COUNT; coeff++) {
            ...
            apu.write((coeff << 4) | SonyS_DSP::Processor::FIR_COEFFICIENTS, param);
        }
        // -------------------------------------------------------------------
        ...
        //       +-----+-----+-----+-----+-----+-----+-----+-----+
        // apu.read(SonyS_DSP::Processor::ENDX, 0);
        // -------------------------------------------------------------------
        // MARK: Stereo output
        // -------------------------------------------------------------------
        short sample[2] = {0, 0};
...
struct SuperSynthWidget : ModuleWidget {
    /// @brief Initialize a new widget.
    ///
    /// @param module the back-end module to interact with
    ///
    explicit SuperSynthWidget(SuperSynth *module) {
        setModule(module);
        ...
        for (unsigned i = 0; i < SonyS_DSP::Processor::VOICE_COUNT; i++) {
            // Frequency
            addInput(createInput<PJ301MPort>(Vec(15, 40 + i * 41), module, SuperSynth::INPUT_VOCT + i));
            addInput(createInput<PJ301MPort>(Vec(45, 40 + i * 41), module, SuperSynth::INPUT_FM + i));
            addParam(createParam<Rogan2PWhite>(Vec(75, 35 + i * 41), module, SuperSynth::PARAM_FREQ + i));
            // Gate
            addInput(createInput<PJ301MPort>(Vec(185, 40 + i * 41), module, SuperSynth::INPUT_GATE + i));
            // Volume - Left
            addInput(createInput<PJ301MPort>(Vec(220, 40 + i * 41), module, SuperSynth::INPUT_VOLUME_L + i));
            auto left = createParam<Rogan2PWhite>(Vec(250, 35 + i * 41), module, SuperSynth::PARAM_VOLUME_L + i);
            left->snap = true;
            addParam(left);
            // Volume - Right
            addInput(createInput<PJ301MPort>(Vec(300, 40 + i * 41), module, SuperSynth::INPUT_VOLUME_R + i));
            auto right = createParam<Rogan2PRed>(Vec(330, 35 + i * 41), module, SuperSynth::PARAM_VOLUME_R + i);
            right->snap = true;
            addParam(right);
            // ADSR - Attack
            addInput(createInput<PJ301MPort>(Vec(390, 40 + i * 41), module, SuperSynth::INPUT_ATTACK + i));
            auto attack = createParam<Rogan2PGreen>(Vec(420, 35 + i * 41), module, SuperSynth::PARAM_ATTACK + i);
            attack->snap = true;
            addParam(attack);
            // ADSR - Decay
            addInput(createInput<PJ301MPort>(Vec(460, 40 + i * 41), module, SuperSynth::INPUT_DECAY + i));
            auto decay = createParam<Rogan2PBlue>(Vec(490, 35 + i * 41), module, SuperSynth::PARAM_DECAY + i);
            decay->snap = true;
            addParam(decay);
            // ADSR - Sustain Level
            addInput(createInput<PJ301MPort>(Vec(530, 40 + i * 41), module, SuperSynth::INPUT_SUSTAIN_LEVEL + i));
            auto sustainLevel = createParam<Rogan2PRed>(Vec(560, 35 + i * 41), module, SuperSynth::PARAM_SUSTAIN_LEVEL + i);
            sustainLevel->snap = true;
            addParam(sustainLevel);
            // ADSR - Sustain Rate
            addInput(createInput<PJ301MPort>(Vec(600, 40 + i * 41), module, SuperSynth::INPUT_SUSTAIN_RATE + i));
            auto sustainRate = createParam<Rogan2PWhite>(Vec(630, 35 + i * 41), module, SuperSynth::PARAM_SUSTAIN_RATE + i);
            sustainRate->snap = true;
            addParam(sustainRate);
            // Phase Modulation
            if (i > 0) {  // phase modulation is not defined for the first voice
                addParam(createParam<CKSS>(Vec(880, 40  + i * 41), module, SuperSynth::PARAM_PM_ENABLE + i));
                addInput(createInput<PJ301MPort>(Vec(900, 40 + i * 41), module, SuperSynth::INPUT_PM_ENABLE + i));
            }
            // Echo Enable
            addParam(createParam<CKSS>(Vec(940, 40  + i * 41), module, SuperSynth::PARAM_ECHO_ENABLE + i));
            addInput(createInput<PJ301MPort>(Vec(960, 40 + i * 41), module, SuperSynth::INPUT_ECHO_ENABLE + i));
            // Noise Enable
            addParam(createParam<CKSS>(Vec(1000, 40  + i * 41), module, SuperSynth::PARAM_NOISE_ENABLE + i));
            addInput(createInput<PJ301MPort>(Vec(1020, 40 + i * 41), module, SuperSynth::INPUT_NOISE_ENABLE + i));
        }

        // Noise Frequency
        addInput(createInput<PJ301MPort>(Vec(115, 40), module, SuperSynth::INPUT_NOISE_FM));
        auto noise = createParam<Rogan2PWhite>(Vec(145, 35), module, SuperSynth::PARAM_NOISE_FREQ);
        noise->snap = true;
        addParam(noise);

        // Echo Delay
        auto echoDelay = createParam<Rogan2PGreen>(Vec(690, 30), module, SuperSynth::PARAM_ECHO_DELAY);
        echoDelay->snap = true;
        addParam(echoDelay);
        addInput(createInput<PJ301MPort>(Vec(700, 80), module, SuperSynth::INPUT_ECHO_DELAY));
        // Echo Feedback
        auto echoFeedback = createParam<Rogan2PGreen>(Vec(740, 30), module, SuperSynth::PARAM_ECHO_FEEDBACK);
        echoFeedback->snap = true;
        addParam(echoFeedback);
        addInput(createInput<PJ301MPort>(Vec(750, 80), module, SuperSynth::INPUT_ECHO_FEEDBACK));

        // Echo Volume - Left channel
        auto echoLeft = createParam<Rogan2PWhite>(Vec(690, 130), module, SuperSynth::PARAM_VOLUME_ECHO + 0);
        echoLeft->snap = true;
        addParam(echoLeft);
        addInput(createInput<PJ301MPort>(Vec(700, 180), module, SuperSynth::INPUT_VOLUME_ECHO + 0));
        // Echo Volume - Right channel
        auto echoRight = createParam<Rogan2PRed>(Vec(740, 130), module, SuperSynth::PARAM_VOLUME_ECHO + 1);
        echoRight->snap = true;
        addParam(echoRight);
        addInput(createInput<PJ301MPort>(Vec(750, 180), module, SuperSynth::INPUT_VOLUME_ECHO + 1));

        // Mixer & Output - Left Channel
        auto volumeLeft = createParam<Rogan2PWhite>(Vec(690, 230), module, SuperSynth::PARAM_VOLUME_MAIN + 0);
        volumeLeft->snap = true;
        addParam(volumeLeft);
        addInput(createInput<PJ301MPort>(Vec(700, 280), module, SuperSynth::INPUT_VOLUME_MAIN + 0));
        addOutput(createOutput<PJ301MPort>(Vec(700, 325), module, SuperSynth::OUTPUT_AUDIO + 0));
        // Mixer & Output - Right Channel
        auto volumeRight = createParam<Rogan2PRed>(Vec(740, 230), module, SuperSynth::PARAM_VOLUME_MAIN + 1);
        volumeRight->snap = true;
        addParam(volumeRight);
        addInput(createInput<PJ301MPort>(Vec(750, 280), module, SuperSynth::INPUT_VOLUME_MAIN + 1));
        addOutput(createOutput<PJ301MPort>(Vec(750, 325), module, SuperSynth::OUTPUT_AUDIO + 1));

        // FIR Coefficients
        for (unsigned i = 0; i < SonyS_DSP::Processor::FIR_COEFFICIENT_COUNT; i++) {
            addInput(createInput<PJ301MPort>(Vec(800, 40 + i * 41), module, SuperSynth::INPUT_FIR_COEFFICIENT + i));
            auto param = createParam<Rogan2PWhite>(Vec(830, 35 + i * 41), module, SuperSynth::PARAM_FIR_COEFFICIENT + i);
            param->snap = true;
            addParam(param);
        }
    }
