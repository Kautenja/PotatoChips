...
struct ChipS_SMP_Gaussian : Module {
 ...
 public:
    ...
    enum ParamIds {
        ENUMS(PARAM_FREQ,          Sony_S_DSP_Gaussian::VOICE_COUNT),
        ENUMS(PARAM_PM_ENABLE,     Sony_S_DSP_Gaussian::VOICE_COUNT),
        ENUMS(PARAM_NOISE_ENABLE,  Sony_S_DSP_Gaussian::VOICE_COUNT),
        PARAM_NOISE_FREQ,
        ENUMS(PARAM_VOLUME_L,      Sony_S_DSP_Gaussian::VOICE_COUNT),
        ENUMS(PARAM_VOLUME_R,      Sony_S_DSP_Gaussian::VOICE_COUNT),
        ENUMS(PARAM_ATTACK,        Sony_S_DSP_Gaussian::VOICE_COUNT),
        ENUMS(PARAM_DECAY,         Sony_S_DSP_Gaussian::VOICE_COUNT),
        ENUMS(PARAM_SUSTAIN_LEVEL, Sony_S_DSP_Gaussian::VOICE_COUNT),
        ENUMS(PARAM_SUSTAIN_RATE,  Sony_S_DSP_Gaussian::VOICE_COUNT),
        ENUMS(PARAM_ECHO_ENABLE,   Sony_S_DSP_Gaussian::VOICE_COUNT),
        PARAM_ECHO_DELAY,
        PARAM_ECHO_FEEDBACK,
        ENUMS(PARAM_VOLUME_ECHO, 2),
        ENUMS(PARAM_VOLUME_MAIN, 2),
        ENUMS(PARAM_FIR_COEFFICIENT, Sony_S_DSP_Gaussian::FIR_COEFFICIENT_COUNT),
        NUM_PARAMS
    };

    /// the indexes of input ports on the module
    enum InputIds {
        ENUMS(INPUT_VOCT,          Sony_S_DSP_Gaussian::VOICE_COUNT),
        ENUMS(INPUT_FM,            Sony_S_DSP_Gaussian::VOICE_COUNT),
        ENUMS(INPUT_PM_ENABLE,     Sony_S_DSP_Gaussian::VOICE_COUNT),
        ENUMS(INPUT_NOISE_ENABLE,  Sony_S_DSP_Gaussian::VOICE_COUNT),
        INPUT_NOISE_FM,
        ENUMS(INPUT_GATE,          Sony_S_DSP_Gaussian::VOICE_COUNT),
        ENUMS(INPUT_VOLUME_L,      Sony_S_DSP_Gaussian::VOICE_COUNT),
        ENUMS(INPUT_VOLUME_R,      Sony_S_DSP_Gaussian::VOICE_COUNT),
        ENUMS(INPUT_ATTACK,        Sony_S_DSP_Gaussian::VOICE_COUNT),
        ENUMS(INPUT_DECAY,         Sony_S_DSP_Gaussian::VOICE_COUNT),
        ENUMS(INPUT_SUSTAIN_LEVEL, Sony_S_DSP_Gaussian::VOICE_COUNT),
        ENUMS(INPUT_SUSTAIN_RATE,  Sony_S_DSP_Gaussian::VOICE_COUNT),
        ENUMS(INPUT_ECHO_ENABLE,   Sony_S_DSP_Gaussian::VOICE_COUNT),
        INPUT_ECHO_DELAY,
        INPUT_ECHO_FEEDBACK,
 ...
 protected:
    ...
    inline void setupSourceDirectory() {
        ...
        apu.write(Sony_S_DSP_Gaussian::ECHO_BUFFER_START_OFFSET, 128);
        ...
        apu.write(Sony_S_DSP_Gaussian::OFFSET_SOURCE_DIRECTORY, ECHO_LENGTH / 0x100);
    ...
    inline void process(const ProcessArgs &args) final {
        ...
        auto dir = reinterpret_cast<Sony_S_DSP_Gaussian::SourceDirectoryEntry*>(&ram[0x7800]);
        ...
        auto block = reinterpret_cast<Sony_S_DSP_Gaussian::BitRateReductionBlock*>(&ram[0x7804]);
        ...
        for (int i = 0; i < Sony_S_DSP_Gaussian::BitRateReductionBlock::NUM_SAMPLES; i++)
            block->samples[i] = 15 + 2 * i;
        // -------------------------------------------------------------------
        // MARK: Flags (Noise Frequency)
        // -------------------------------------------------------------------
        uint8_t noise = params[PARAM_NOISE_FREQ].getValue();
        apu.write(Sony_S_DSP_Gaussian::FLAGS, noise);
        // -------------------------------------------------------------------
        // MARK: Gate input
        // -------------------------------------------------------------------
        ...
        // -------------------------------------------------------------------
        apu.write(Sony_S_DSP_Gaussian::ECHO_FEEDBACK, params[PARAM_ECHO_FEEDBACK].getValue());
        apu.write(Sony_S_DSP_Gaussian::ECHO_DELAY, params[PARAM_ECHO_DELAY].getValue());
        // echo enable
        uint8_t echo_enable = 0;
        for (unsigned voice = 0; voice < Sony_S_DSP_Gaussian::VOICE_COUNT; voice++)
            echo_enable |= static_cast<uint8_t>(params[PARAM_ECHO_ENABLE + voice].getValue()) << voice;
        apu.write(Sony_S_DSP_Gaussian::ECHO_ENABLE, echo_enable);
        // -------------------------------------------------------------------
        // MARK: Noise Enable
        // -------------------------------------------------------------------
        ...
        apu.write(Sony_S_DSP_Gaussian::NOISE_ENABLE, noise_enable);
        // -------------------------------------------------------------------
        // MARK: Pitch Modulation
        // -------------------------------------------------------------------
        uint8_t pitch_modulation = 0;
        // start from 1 because there is no pitch modulation for the first channel
        for (unsigned voice = 1; voice < Sony_S_DSP_Gaussian::VOICE_COUNT; voice++)
            pitch_modulation |= static_cast<uint8_t>(params[PARAM_PM_ENABLE + voice].getValue()) << voice;
        apu.write(Sony_S_DSP_Gaussian::PITCH_MODULATION, pitch_modulation);
        // -------------------------------------------------------------------
        // MARK: Main Volume & Echo Volume
        // -------------------------------------------------------------------
        apu.write(Sony_S_DSP_Gaussian::MAIN_VOLUME_LEFT,  params[PARAM_VOLUME_MAIN + 0].getValue());
        apu.write(Sony_S_DSP_Gaussian::MAIN_VOLUME_RIGHT, params[PARAM_VOLUME_MAIN + 1].getValue());
        apu.write(Sony_S_DSP_Gaussian::ECHO_VOLUME_LEFT,  params[PARAM_VOLUME_ECHO + 0].getValue());
        apu.write(Sony_S_DSP_Gaussian::ECHO_VOLUME_RIGHT, params[PARAM_VOLUME_ECHO + 1].getValue());
        // -------------------------------------------------------------------
        ...
        for (unsigned voice = 0; voice < Sony_S_DSP_Gaussian::VOICE_COUNT; voice++) {
            ...
            auto pitch16bit = Sony_S_DSP_Gaussian::convert_pitch(frequency);
            // set the 14-bit pitch value to the cascade of two RAM slots
            apu.write(mask | Sony_S_DSP_Gaussian::PITCH_LOW,  0xff &  pitch16bit     );
            apu.write(mask | Sony_S_DSP_Gaussian::PITCH_HIGH, 0xff & (pitch16bit >> 8));
            // ---------------------------------------------------------------
            ...
            //
            // apu.write(mask | Sony_S_DSP_Gaussian::GAIN, 64);
            // ---------------------------------------------------------------
            // MARK: ADSR
            // ---------------------------------------------------------------
            ...
            apu.write(mask | Sony_S_DSP_Gaussian::ADSR_1, adsr1);
            ...
            apu.write(mask | Sony_S_DSP_Gaussian::ADSR_2, adsr2);
            // ---------------------------------------------------------------
            // MARK: ADSR Output
            // ---------------------------------------------------------------
            ...
            // apu.read(mask | Sony_S_DSP_Gaussian::ENVELOPE_OUT, 0);
            // ---------------------------------------------------------------
            ...
            //
            // 8-bit signed value
            // apu.read(mask | Sony_S_DSP_Gaussian::WAVEFORM_OUT, 0);
            // ---------------------------------------------------------------
            // MARK: Amplifier Volume
            // ---------------------------------------------------------------
            apu.write(mask | Sony_S_DSP_Gaussian::VOLUME_LEFT,  params[PARAM_VOLUME_L + voice].getValue());
            apu.write(mask | Sony_S_DSP_Gaussian::VOLUME_RIGHT, params[PARAM_VOLUME_R + voice].getValue());
        }
        // -------------------------------------------------------------------
        ...
        for (unsigned coeff = 0; coeff < Sony_S_DSP_Gaussian::FIR_COEFFICIENT_COUNT; coeff++) {
            ...
            apu.write((coeff << 4) | Sony_S_DSP_Gaussian::FIR_COEFFICIENTS, param);
        }
        // -------------------------------------------------------------------
        ...
        //       +-----+-----+-----+-----+-----+-----+-----+-----+
        // apu.read(Sony_S_DSP_Gaussian::ENDX, 0);
        // -------------------------------------------------------------------
        // MARK: Stereo output
        // -------------------------------------------------------------------
        short sample[2] = {0, 0};
...
struct ChipS_SMP_GaussianWidget : ModuleWidget {
    ...
    explicit ChipS_SMP_GaussianWidget(ChipS_SMP_Gaussian *module) {
        ...
        for (unsigned i = 0; i < Sony_S_DSP_Gaussian::VOICE_COUNT; i++) {
            // Frequency
            addInput(createInput<PJ301MPort>(Vec(15, 40 + i * 41), module, ChipS_SMP_Gaussian::INPUT_VOCT + i));
            addInput(createInput<PJ301MPort>(Vec(45, 40 + i * 41), module, ChipS_SMP_Gaussian::INPUT_FM + i));
            addParam(createParam<Rogan2PSNES>(Vec(75, 35 + i * 41), module, ChipS_SMP_Gaussian::PARAM_FREQ + i));
            // Gate
            addInput(createInput<PJ301MPort>(Vec(185, 40 + i * 41), module, ChipS_SMP_Gaussian::INPUT_GATE + i));
            // Volume - Left
            addInput(createInput<PJ301MPort>(Vec(220, 40 + i * 41), module, ChipS_SMP_Gaussian::INPUT_VOLUME_L + i));
            auto left = createParam<Rogan2PWhite>(Vec(250, 35 + i * 41), module, ChipS_SMP_Gaussian::PARAM_VOLUME_L + i);
            left->snap = true;
            addParam(left);
            // Volume - Right
            addInput(createInput<PJ301MPort>(Vec(300, 40 + i * 41), module, ChipS_SMP_Gaussian::INPUT_VOLUME_R + i));
            auto right = createParam<Rogan2PRed>(Vec(330, 35 + i * 41), module, ChipS_SMP_Gaussian::PARAM_VOLUME_R + i);
            right->snap = true;
            addParam(right);
            // ADSR - Attack
            addInput(createInput<PJ301MPort>(Vec(390, 40 + i * 41), module, ChipS_SMP_Gaussian::INPUT_ATTACK + i));
            auto attack = createParam<Rogan2PGreen>(Vec(420, 35 + i * 41), module, ChipS_SMP_Gaussian::PARAM_ATTACK + i);
            attack->snap = true;
            addParam(attack);
            // ADSR - Decay
            addInput(createInput<PJ301MPort>(Vec(460, 40 + i * 41), module, ChipS_SMP_Gaussian::INPUT_DECAY + i));
            auto decay = createParam<Rogan2PBlue>(Vec(490, 35 + i * 41), module, ChipS_SMP_Gaussian::PARAM_DECAY + i);
            decay->snap = true;
            addParam(decay);
            // ADSR - Sustain Level
            addInput(createInput<PJ301MPort>(Vec(530, 40 + i * 41), module, ChipS_SMP_Gaussian::INPUT_SUSTAIN_LEVEL + i));
            auto sustainLevel = createParam<Rogan2PRed>(Vec(560, 35 + i * 41), module, ChipS_SMP_Gaussian::PARAM_SUSTAIN_LEVEL + i);
            sustainLevel->snap = true;
            addParam(sustainLevel);
            // ADSR - Sustain Rate
            addInput(createInput<PJ301MPort>(Vec(600, 40 + i * 41), module, ChipS_SMP_Gaussian::INPUT_SUSTAIN_RATE + i));
            auto sustainRate = createParam<Rogan2PWhite>(Vec(630, 35 + i * 41), module, ChipS_SMP_Gaussian::PARAM_SUSTAIN_RATE + i);
            sustainRate->snap = true;
            addParam(sustainRate);
            // Phase Modulation
            if (i > 0) {  // phase modulation is not defined for the first voice
                addParam(createParam<CKSS>(Vec(880, 40  + i * 41), module, ChipS_SMP_Gaussian::PARAM_PM_ENABLE + i));
                addInput(createInput<PJ301MPort>(Vec(900, 40 + i * 41), module, ChipS_SMP_Gaussian::INPUT_PM_ENABLE + i));
            }
            // Echo Enable
            addParam(createParam<CKSS>(Vec(940, 40  + i * 41), module, ChipS_SMP_Gaussian::PARAM_ECHO_ENABLE + i));
            addInput(createInput<PJ301MPort>(Vec(960, 40 + i * 41), module, ChipS_SMP_Gaussian::INPUT_ECHO_ENABLE + i));
            // Noise Enable
            addParam(createParam<CKSS>(Vec(1000, 40  + i * 41), module, ChipS_SMP_Gaussian::PARAM_NOISE_ENABLE + i));
            addInput(createInput<PJ301MPort>(Vec(1020, 40 + i * 41), module, ChipS_SMP_Gaussian::INPUT_NOISE_ENABLE + i));
        }

        // Noise Frequency
        addInput(createInput<PJ301MPort>(Vec(115, 40), module, ChipS_SMP_Gaussian::INPUT_NOISE_FM));
        auto noise = createParam<Rogan2PSNES>(Vec(145, 35), module, ChipS_SMP_Gaussian::PARAM_NOISE_FREQ);
        noise->snap = true;
        addParam(noise);

        // Echo Delay
        auto echoDelay = createParam<Rogan2PGreen>(Vec(690, 30), module, ChipS_SMP_Gaussian::PARAM_ECHO_DELAY);
        echoDelay->snap = true;
        addParam(echoDelay);
        addInput(createInput<PJ301MPort>(Vec(700, 80), module, ChipS_SMP_Gaussian::INPUT_ECHO_DELAY));
        // Echo Feedback
        auto echoFeedback = createParam<Rogan2PGreen>(Vec(740, 30), module, ChipS_SMP_Gaussian::PARAM_ECHO_FEEDBACK);
        echoFeedback->snap = true;
        addParam(echoFeedback);
        addInput(createInput<PJ301MPort>(Vec(750, 80), module, ChipS_SMP_Gaussian::INPUT_ECHO_FEEDBACK));

        // Echo Volume - Left channel
        auto echoLeft = createParam<Rogan2PWhite>(Vec(690, 130), module, ChipS_SMP_Gaussian::PARAM_VOLUME_ECHO + 0);
        echoLeft->snap = true;
        addParam(echoLeft);
        addInput(createInput<PJ301MPort>(Vec(700, 180), module, ChipS_SMP_Gaussian::INPUT_VOLUME_ECHO + 0));
        // Echo Volume - Right channel
        auto echoRight = createParam<Rogan2PRed>(Vec(740, 130), module, ChipS_SMP_Gaussian::PARAM_VOLUME_ECHO + 1);
        echoRight->snap = true;
        addParam(echoRight);
        addInput(createInput<PJ301MPort>(Vec(750, 180), module, ChipS_SMP_Gaussian::INPUT_VOLUME_ECHO + 1));

        // Mixer & Output - Left Channel
        auto volumeLeft = createParam<Rogan2PWhite>(Vec(690, 230), module, ChipS_SMP_Gaussian::PARAM_VOLUME_MAIN + 0);
        volumeLeft->snap = true;
        addParam(volumeLeft);
        addInput(createInput<PJ301MPort>(Vec(700, 280), module, ChipS_SMP_Gaussian::INPUT_VOLUME_MAIN + 0));
        addOutput(createOutput<PJ301MPort>(Vec(700, 325), module, ChipS_SMP_Gaussian::OUTPUT_AUDIO + 0));
        // Mixer & Output - Right Channel
        auto volumeRight = createParam<Rogan2PRed>(Vec(740, 230), module, ChipS_SMP_Gaussian::PARAM_VOLUME_MAIN + 1);
        volumeRight->snap = true;
        addParam(volumeRight);
        addInput(createInput<PJ301MPort>(Vec(750, 280), module, ChipS_SMP_Gaussian::INPUT_VOLUME_MAIN + 1));
        addOutput(createOutput<PJ301MPort>(Vec(750, 325), module, ChipS_SMP_Gaussian::OUTPUT_AUDIO + 1));

        // FIR Coefficients
        for (unsigned i = 0; i < Sony_S_DSP_Gaussian::FIR_COEFFICIENT_COUNT; i++) {
            addInput(createInput<PJ301MPort>(Vec(800, 40 + i * 41), module, ChipS_SMP_Gaussian::INPUT_FIR_COEFFICIENT + i));
            auto param = createParam<Rogan2PWhite>(Vec(830, 35 + i * 41), module, ChipS_SMP_Gaussian::PARAM_FIR_COEFFICIENT + i);
            param->snap = true;
            addParam(param);
        }
    }
