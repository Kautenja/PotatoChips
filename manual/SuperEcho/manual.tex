...
\begin{itemize}
  ...
  \item \textbf{16-bit Inputs:} The BRR decoder has been remove in this emulation. Instead, input audio is sampled directly as 16-bit PCM. A separate module S-SMP(BRR) can be cascaded into Super Echo to introduce authentic BRR down-sampling effects to the signal path.
  \item \textbf{Gaussian Low-Pass Filter Override:} The Gaussian low-pass filter has been removed in this emulation. A separate module, S-SMP(Gaussian), can be used in cascade with Super Echo to produce a more authentic SNES sound.
\end{itemize}
